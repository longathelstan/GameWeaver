...
- [x] **UI Library:** Cài đặt Shadcn UI cho `client`. Khởi tạo các component cơ bản: Button, Input, Card, Checkbox, Dialog, ScrollArea, Textarea, Tabs.
- [x] **Database Setup:** Cài đặt Prisma và kết nối với PostgreSQL. Tạo schema `User`, `Project`, `TextbookData` (lưu raw JSON), và `QuestionBank`.
- [x] **AI Configuration:** Cài đặt `@google/generative-ai` và `langchain` trong `server`. Tạo file config để load `GEMINI_API_KEY`.

#### Phase 2: Backend - RAG & Data Processing (Core)
- [x] **Endpoint `/api/ingest-data`:** Viết API nhận file JSON (SGK). Sử dụng LangChain để chia nhỏ (chunk) dữ liệu văn bản từ JSON và lưu vào Vector Store (hoặc lưu text thuần nếu dữ liệu nhỏ) để phục vụ RAG.
- [x] **Endpoint `/api/map-content`:** Viết API sử dụng Gemini để phân tích file JSON đã upload. Trả về cấu trúc cây: `Chương -> Bài học -> Trang` để frontend hiển thị cây thư mục lựa chọn.
- [x] **RAG Utility Function:** Viết hàm `retrieveContext(topic)`: Tìm kiếm dữ liệu liên quan trong vector store dựa trên nội dung giáo viên chọn.

#### Phase 3: Backend - Question Generation & Game Logic
- [x] **Endpoint `/api/generate-questions`:**
...
- [x] **Endpoint `/api/generate-game-code`:**
...
- [x] **Global State:** Cài đặt Zustand store để lưu trữ: `currentStep`, `selectedTopics`, `generatedQuestions`, `gameConfig`, `generatedCode`.
- [x] **Step 1: Input & Mapping UI:**
...
- [x] **Step 2: Question Management (Human-in-the-loop):**
...
- [x] **Step 4: Code Generation & Preview:**
